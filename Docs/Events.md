...
세션은 `NpcEventManager`의 활성 필드 한 묶음으로 관리하도록 구현했습니다. 최근 32개 요청 ID로 중복을 확인하고, 현재 세션 상태와 진행 조건도 함께 검사하도록 했습니다. 보관 범위를 벗어난 요청은 ID 이력만으로 중복 여부를 판단할 수 없습니다. FIFO 앞 원소 삭제 시 이동할 원소 수도 보관 개수로 제한됩니다.
